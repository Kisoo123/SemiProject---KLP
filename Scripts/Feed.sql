...
(SELECT ROWNUM AS RNUM,B.* FROM 
	(SELECT ROWNUM AS REALNUM, INB.* FROM 
		(SELECT * FROM FEED JOIN (SELECT FEED_NO, LISTAGG(FILE_PATH, ',') WITHIN GROUP(ORDER BY file_no) AS filePath, COUNT(1) FROM FEED_FILE GROUP BY FEED_NO HAVING COUNT(1) > 1) USING (FEED_NO) ORDER BY WRITEDATE DESC) INB )B)
WHERE REALNUM BETWEEN 11 AND 20
